...
pub struct Master{
    pub mechanical_world: DefaultMechanicalWorld<f32>,    //N/M types are somehow not right??? Maybe give specific types???
    ...
    pub player: Player,
    pub count: i32,                     //test vraible to only the game run a fixed amount of ticks.
}
//TODO: implement structs Player and Enemy

impl Master{
    pub fn new(self, ctx: Context) -> Self{
        ...
        Master{
            ...
            gameObjList: Vec::new(),
            player: Player::new(self.bodies, self.colliders),
            count: 0,
        }
    }
